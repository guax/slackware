 ...
 *
 * Modified 2009 by Henrique Grolli Bassotto <henrique.grolli@gmail.com>
 * to ignore case on comparing channels to capture.
 */
	...
	if (!strcmp("B", option)) {
			...
			while (cur) {
				...
				if (bridged) {
					...
					if ((cur != chan) && (bridged != chan) &&
						...
						!strncasecmp(pat, bridged->name, strlen(pat))) {
						/*
						ast_verbose(VERBOSE_PREFIX_4
		...
		while (pat) {
			...
			while (cur) {
				...
				if ((cur != chan) && 
					(cur->_state == chanstate) &&
					!strncasecmp(pat, cur->name, strlen(pat))) {
					/*
					ast_verbose(VERBOSE_PREFIX_4
							"find_matching_channel: "
							"found channel='%s'\n",
